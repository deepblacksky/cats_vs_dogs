...
def get_file(file_dir, ratio):
    ...
    :param ratio:test data / total data
    :return:list(directory) of train image and train label, also, test image and test label 
    """
    cats = []
    dogs = []
    label_cats = []
    ...
    num_examples = len(all_label_list)
    num_test_examples = math.ceil(num_examples * ratio)
    num_train_examples = num_examples - num_test_examples

    train_images = all_image_list[0:num_train_examples]
    train_labels = all_label_list[0:num_train_examples]
    train_labels = [int(float(i)) for i in train_labels]

    test_images = all_image_list[num_train_examples:-1]
    test_labels = all_label_list[num_train_examples:-1]
